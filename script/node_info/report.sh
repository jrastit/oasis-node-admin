...
if [[ $ERROR != "" ]] ; then
	echo -e "$OASIS_NODE_NAME $SERVICE_STATUS ${RED}$INFO $DISPLAY_TIME $REGISTRATION $VALIDATOR ${NC}$FREE_DISK_INFO $OASIS_NODE_ADDR $OASIS_NODE_PORT $OASIS_P2P_PORT"
	echo -e "${RED}$ERROR${NC}" | sed -z '$ s/\n$//'
else
	echo -e "$OASIS_NODE_NAME $SERVICE_STATUS ${GREEN}$INFO $DISPLAY_TIME $REGISTRATION $VALIDATOR ${NC}$FREE_DISK_INFO $OASIS_NODE_ADDR $OASIS_NODE_PORT $OASIS_P2P_PORT"	
fi
