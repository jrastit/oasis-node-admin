...
for NETWORK_DIR in $LOCAL_CONFIG_DIR/network/*; do
	if [ -d $NETWORK_DIR ]; then
		...
		log_cmd $REMOTE_SYNC $OASIS_NODE_SSH:$NETWORK_DIR_REMOTE/network_config.sh $NETWORK_DIR
		for TYPE_DIR in $NETWORK_DIR/*; do
			if [ -d $TYPE_DIR ]; then
				...
				log_cmd $REMOTE_SYNC $OASIS_NODE_SSH:$TYPE_DIR_REMOTE/type_config.sh $TYPE_DIR
			fi
		done
	fi
done
...
for NODE_DIR in $LOCAL_CONFIG_DIR/node/*; do	
	if [ -d $NODE_DIR ]; then
		...
		log_cmd $REMOTE_SYNC $OASIS_NODE_SSH:$NODE_DIR_REMOTE/node_config.sh $NODE_DIR
	fi
done
