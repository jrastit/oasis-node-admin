...
if [ -n "$CUSTOM_OASIS_P2P_PORT" ] ; then 
	OASIS_P2P_PORT=$CUSTOM_OASIS_P2P_PORT
fi
...
if [ -n "$CUSTOM_OASIS_NODE_PRUNE_CONSENSUS_NUM_KEPT" ] ; then 
	OASIS_NODE_PRUNE_CONSENSUS_NUM_KEPT=$CUSTOM_OASIS_NODE_PRUNE_CONSENSUS_NUM_KEPT
fi

if [ -n "$CUSTOM_OASIS_NODE_PRUNE_RUNTIME_NUM_KEPT" ] ; then 
	OASIS_NODE_PRUNE_RUNTIME_NUM_KEPT=$CUSTOM_OASIS_NODE_PRUNE_RUNTIME_NUM_KEPT
fi

if [ -n "$CUSTOM_OASIS_NODE_LOG_LEVEL" ] ; then 
	LOG_LEVEL=$CUSTOM_OASIS_NODE_LOG_LEVEL
fi
...
if [ -z "$OASIS_NODE_SSH" ]; then
	OASIS_NODE_SSH=$OASIS_NODE_SSH_NAME@$OASIS_NODE_ADDR
fi

if [ -z "$OASIS_NODE_SSH_ADMIN" ]; then
...
OASIS_NODE_CLI="$SSHCMD $OASIS_NODE_DIR/cli/$OASIS_CLI_DIR/oasis"
