...
if [ -n "$CUSTOM_NETWORK_ROOT_DIR" ] ; then 
	NETWORK_ROOT_DIR=$CUSTOM_NETWORK_ROOT_DIR
fi
NETWORK_DIR="$NETWORK_ROOT_DIR/$NETWORK_NODE_NAME"
...
if [ -n "$NETWORK_HOST_ADMIN" ]; then
	REMOTE_CMD_ADMIN="ssh -o LogLevel=QUIET -t -4 $NETWORK_HOST_ADMIN sudo"
else 
